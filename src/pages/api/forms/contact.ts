...
function buildEmailBody(data: ContactFormData): string {
  return `
    ...
    <hr>
    <p><small>Submitted from: ${data.source_page || 'contact'} | ${new Date().toISOString()}</small></p>
  `;
}
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const subject = `New Contact Inquiry — ${data.first_name} ${data.last_name} — ${data.phone}`;
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Your inquiry has been received. We will follow up with you shortly.',
